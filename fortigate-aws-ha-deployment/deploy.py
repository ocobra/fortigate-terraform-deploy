...
class TerraformManager:
    ...
    def configure_backend(self) -> bool:
        ...
        if self.backend_config.backend_type == "s3":
            click.echo("📦 Configuring S3 backend with DynamoDB locking...")
            ...
            if not all([
                ...
                click.echo("❌ S3 backend requires: bucket, key, region, and dynamodb_table")
                return False
...
'''
            ...
            click.echo(f"✅ S3 backend configuration written to {self.backend_tf_file}")
            click.echo(f"   Bucket: {self.backend_config.s3_bucket}")
            click.echo(f"   Key: {self.backend_config.s3_key}")
            click.echo(f"   DynamoDB Table: {self.backend_config.dynamodb_table}")
            
        else:  # local backend
            click.echo("💾 Using local backend (terraform.tfstate)")
            
            # Remove backend.tf if it exists (to use default local backend)
            if self.backend_tf_file.exists():
                click.echo(f"   Removing existing backend.tf to use local state")
                self.backend_tf_file.unlink()
        
        return True
    
    def init(self, backend_config: Optional[Dict[str, str]] = None, reconfigure: bool = False) -> bool:
...
class DeploymentEngine:
    ...
    def __init__(self, config: DeploymentConfig):
        ...
        self.terraform = TerraformManager(self.terraform_dir, config.backend)
    ...
    def plan(self) -> bool:
        ...
        # Configure backend
        if not self.terraform.configure_backend():
            return False
        
        # Generate tfvars file
        self.terraform.generate_tfvars(self.config)
        
        # Initialize Terraform (with reconfigure if switching backends)
        reconfigure = self.config.backend.backend_type == "s3"
        if not self.terraform.init(reconfigure=reconfigure):
            return False
...
def prompt_backend_config() -> BackendConfig:
    """Prompt user for Terraform backend configuration"""
    click.echo("\n💾 Terraform Backend Configuration")
    click.echo("=" * 50)
    ...
    if backend_type.lower() == "s3":
        ...
        click.echo("Note: S3 bucket and DynamoDB table should be created using terraform/bootstrap/")
        click.echo("")
        
        s3_bucket = click.prompt("S3 bucket name (from bootstrap output)")
        s3_key = click.prompt("S3 state file key", default="fortigate-ha/terraform.tfstate")
        s3_region = click.prompt("S3 bucket region", default="us-east-1")
        dynamodb_table = click.prompt("DynamoDB table name (from bootstrap output)", default="fortigate-terraform-locks")
        
        encrypt = click.confirm("Encrypt state file?", default=True)
        
        use_profile = click.confirm("Use AWS profile for backend?", default=True)
        s3_profile = None
        if use_profile:
            s3_profile = click.prompt("AWS profile name", default="default")
        
        use_kms = click.confirm("Use KMS encryption?", default=False)
        kms_key_id = None
        if use_kms:
            kms_key_id = click.prompt("KMS Key ID or ARN")
        ...
        click.echo("\n💾 Using local backend (terraform.tfstate in terraform directory)")
        return BackendConfig(backend_type="local")
...
def main(config: Optional[str], plan_only: bool, destroy: bool, save_config: Optional[str], 
    ...
    # Handle bootstrap-info option
    if bootstrap_info:
        click.echo("\n📦 Terraform State Management Bootstrap")
        click.echo("=" * 50)
        click.echo("\nBefore using S3 backend, you must first create the S3 bucket and DynamoDB table.")
        click.echo("\nSteps:")
        click.echo("1. Navigate to bootstrap directory:")
        click.echo("   cd terraform/bootstrap")
        click.echo("\n2. Copy and configure variables:")
        click.echo("   cp terraform.tfvars.example terraform.tfvars")
        click.echo("   # Edit terraform.tfvars with your values")
        click.echo("\n3. Run bootstrap:")
        ...
        click.echo("\nExample:")
        click.echo("  python3 deploy.py --backend=s3 \\")
        click.echo("    --s3-bucket=your-bucket-name \\")
        click.echo("    --s3-region=us-east-1 \\")
        click.echo("    --dynamodb-table=fortigate-terraform-locks")
        click.echo("\nFor more details, see: terraform/bootstrap/README.md")
        return
    ...
    if config:
        ...
        # Backend configuration - use CLI options if provided, otherwise prompt
        if backend:
            if backend.lower() == "s3":
                if not s3_bucket:
                    click.echo("❌ --s3-bucket is required when using --backend=s3")
                    click.echo("💡 Run with --bootstrap-info to see setup instructions")
                    sys.exit(1)
                ...
                backend_config = BackendConfig(
                    ...
                    s3_bucket=s3_bucket,
                    s3_key=s3_key,
                    s3_region=s3_region or aws_config.region,
                    dynamodb_table=dynamodb_table,
                    encrypt=True,
                    s3_profile=aws_config.profile
                )
                click.echo(f"\n✅ Using S3 backend: s3://{s3_bucket}/{s3_key}")
            else:
                backend_config = BackendConfig(backend_type="local")
                click.echo("\n✅ Using local backend")
        else:
            backend_config = prompt_backend_config()
